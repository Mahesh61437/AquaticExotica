...
const isAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!req.session.userId) {
      console.debug("Admin access denied: No user ID in session");
      return res.status(401).json({ message: "Unauthorized: Please log in first" });
    }

    console.debug(`Admin middleware: Checking admin status for user ID: ${req.session.userId}`);
    ...
    try {
      ...
      if (!user) {
        console.warn(`Admin access denied: User with ID ${req.session.userId} not found in database`);
      ...
      if (!user.isAdmin) {
        console.warn(`Admin access denied: User ${user.username} (${user.email}) is not an admin`);
        return res.status(403).json({ message: "Forbidden: Admin access required" });
      }
      
      console.info(`Admin access granted for user: ${user.username} (${user.email})`);
      // Everything is good, proceed
      next();
    } catch (dbError) {
      console.error("Admin middleware: Database error when retrieving user:", { error: dbError });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    try {
      ...
      if (user.email === 'mahesh@aquaticexotica.com') {
        isAdmin = true;
        console.log(`User ${user.username} (${user.email}) isAdmin=${isAdmin}`);
        console.log(`ADMIN CHECK: User ${user.username} via email force-set admin status to true`);
        
        // If mahesh user exists but isn't an admin in the database, make them an admin
        if (!user.isAdmin) {
          try {
            await db.execute(`
              ...
              WHERE id = ${user.id}
            `);
            console.log(`Updated user ${user.username} to have admin privileges in database`);
          } catch (err) {
      ...
      // Return user with possibly updated admin status
      res.json({
        ...userWithoutPassword,
        isAdmin
      });
  ...
  app.get("/api/auth/me", async (req: Request, res: Response) => {
    try {
      ...
      // Hardcoded admin check for the mahesh account with right email
      let isAdmin = user.isAdmin;
      if (userId === 1 && user.email === 'mahesh@aquaticexotica.com') {
        isAdmin = true;
        console.log(`API route-level admin check: Force-setting admin=true for user ${user.username}`);
      }
      
      // Remove password from response
      const { password, ...userToSend } = user;
      
      // Create the final user object with the correct admin status
      const userWithoutPassword = {
        ...userToSend,
        isAdmin: isAdmin
      };
      
      console.log(`GET /api/auth/me - Responding with user: ${user.username}, admin: ${userWithoutPassword.isAdmin}`);
  ...
  app.post("/api/orders", async (req, res) => {
    try {
      ...
      try {
        ...
        // If database insert fails, return an error
  ...
  app.patch("/api/admin/orders/:id", isAdmin, async (req, res) => {
    try {
      ...
      // Send email notification if status changed to processing or shipped
      if (status === 'processing' || status === 'shipped') {
        try {
          const emailSent = await sendOrderStatusUpdate(updatedOrder, status);
          console.log(`Order status update email for order #${id} ${emailSent ? 'sent' : 'failed'}`);
        } catch (emailError) {
          console.error(`Failed to send order status update email for order #${id}:`, emailError);
