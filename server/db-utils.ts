...
export async function getUserByEmailViaSQL(email: string): Promise<User | null> {
  try {
    // Build the query to get user data
    const query = `
      SELECT id, username, email, password, full_name, created_at
      FROM users 
      WHERE email = '${email}'
    ...
    // HARDCODED: admin for mahesh@aquaticexotica.com user (id 1 and correct email)
    const isAdmin = (dbUser.id === 1 && email === 'mahesh@aquaticexotica.com');
    console.log(`User ${dbUser.username} (${email}) isAdmin=${isAdmin}`);
    ...
    return {
      ...
      fullName: dbUser.full_name,
      createdAt: dbUser.created_at,
      // Force admin=true for the mahesh account with right email
      isAdmin: isAdmin
    };
  } catch (error) {
